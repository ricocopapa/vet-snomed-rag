...
생성 시각: 2026-04-19 17:54:34

## 요약 테이블

| 메트릭 | none (기준선) | Gemini 2.5 Flash | Claude Sonnet 4.6 (미실행) |
| --- | --- | --- | --- |
| 11쿼리 PASS율 | 6/10 | 10/10 | API 키 미설정으로 실측 생략 |
| T7 (feline diabetes→73211009) | ✗ (없음) | ✓ (rank #1) | 미실행 |
| 총 비용 | $0.00000 | $0.00000 | 미실행 |
| 평균 레이턴시 | 1247ms | 1063ms | 미실행 |
...
| T4 | pancreatitis in dog | 75694006 | #2 | PASS | #1 | PASS | API 키 미설정으로 실측 생략 |
...
| T7 | feline diabetes | 73211009 | 없음 | FAIL | #1 | PASS | API 키 미설정으로 실측 생략 |
| T8 | diabetes mellitus type 1 | 46635009 | #1 | PASS | #1 | PASS | API 키 미설정으로 실측 생략 |
| T9 | 고양이 당뇨 | 73211009 | 없음 | FAIL | #1 | PASS | API 키 미설정으로 실측 생략 |
| T10 | 개 췌장염 | 75694006 | 없음 | FAIL | #1 | PASS | API 키 미설정으로 실측 생략 |
| T11 | 고양이 범백혈구감소증 SNOMED 코드 | 339181000009108 | 없음 | FAIL | #1 | PASS | API 키 미설정으로 실측 생략 |
